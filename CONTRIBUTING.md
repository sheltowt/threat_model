...
`tmac-core` and `tmac-rules` each export a `/browser` entry carrying only the half
...
`tmac-core/browser` from any package that the editor also loads.
...
tmac serve --write      # the built editor against a real file
```
