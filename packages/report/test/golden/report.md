...
condition because the model does not record the control it asks about, so tmc reports
...
Generated by tmc from `the model` at 2026-09-06T12:00:00Z. Risk ids are derived from the rule and the model ids it concerns, so they survive a regeneration and can be tracked in `risk_tracking`.
