...
import { buildGraph, builtinCatalog, parseModelText } from '@tmc/core/browser';
import { analyze, builtinRules } from '@tmc/rules/browser';
...
function nodeImportsReachableFrom(entry: string): Map<string, string[]> {
  ...
  const visit = (file: string): void => {
    ...
    for (const specifier of specifiers) {
      ...
      else if (specifier.startsWith('@tmc/core/browser')) {
        visit(join(ROOT, 'packages/core/dist/browser.js'));
      } else if (specifier === '@tmc/core') {
...
describe('browser entry point', () => {
  it('reaches no Node built-in, including through @tmc/core', () => {
...
describe('the compiled-in rule library', () => {
  ...
  it('matches what the filesystem loader produces', async () => {
    ...
    const { loadRules } = await import('@tmc/rules');
...
describe('analysing entirely in memory', () => {
  it('produces the same findings as the filesystem path', async () => {
    ...
    const { loadModel } = await import('@tmc/core');
    const { loadRules } = await import('@tmc/rules');
